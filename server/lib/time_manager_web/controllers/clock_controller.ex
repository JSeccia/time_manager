defmodule TimeManagerWeb.ClockController do
  ...
  def create_by_username(conn, %{"username" => username}) do
    ...
    if previous_clock == nil || previous_clock.status == false do
      ...
      with {:ok, %Clock{} = clock} <- Api.create_clock(clock_params) do
        conn
        |> put_status(:created)
        |> put_resp_header("location", Routes.clock_path(conn, :create_by_username, clock))
      ...
      working_time_params = Map.put(working_time_params, "user_id", user.id)
