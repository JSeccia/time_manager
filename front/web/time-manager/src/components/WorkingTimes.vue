...
export default {
  ...
  methods: {
    ...
    // goWorkingTime() {
    //   axios
    //     .get(`http://192.168.76.161:4000/api/working_times/${this.currentUserId}/${this.currentWorkingTimeId}`)
    //     .then((response) => {
    //       if (response.data.data.length > 0) {
    //         this.workingTimes = response.data.data;
    //         console.log(this.workingTimes);

    //         let workingtimeids = this.workingTimes.filter((workingTime) => workingTime.id)
    //         console.log(workingtimeids);
    //         let currentWorkingTimeId = this.workingTimes.map((workingTime) => workingTime.id);
    //         console.log(currentWorkingTimeId);
    //         let findworkingtimeid = this.workingTimes.find((workingTime) => workingTime.id === this.workingtimeid)
    //         console.log(findworkingtimeid);
    //         this.currentWorkingTimeId = findworkingtimeid;
    //         let WTUid = this.currentWorkingTimeId;
    //         this.$router.push(`/workingtimes/${this.currentUserId}/${WTUid}`);
    //       } else {
    //         window.alert("No employee or working times found");
    //       }
    //     }); 
      ...
      axios
        ...
        .then((response) => {
          if (response.data.data.length > 0) {
            ...
            console.log(this.currentWorkingTimeId);
            this.$router.push(`/workingtimes/${this.currentUserId}/${this.currentWorkingTimeId}`);
          }
        });
    },
  },
