<template>
    <main  class="main_teams">
            ...
            <table class="teams-table">
                ...
                <tbody>
                    <tr class="T_items" v-for="team in teams" :key="team.id">
                        <td><input type="button" class="select_team" :value="team.id" @click.prevent="goTeamShow" /></td>
                        <td>{{ team.manager }}</td>
        ...
        <form id="create-form" v-if="isTeamCreateBtnClicked" @submit.prevent="createTeam">
            <fieldset>
                <h2 class="fs-title">Create a new Team</h2>
                <label for="manager_input">Manager Id</label>
                <input type="text" v-model="manager" id="manager_input" name="manager_input" />                
                <br />
...
export default {
    ...
    methods: {
        ...
        goTeamShow(e) {
            axios
            .get(`/api/teams/${this.currentTeamSelected}`)
            .then((response) => {
